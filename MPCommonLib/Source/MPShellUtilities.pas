...
function SpecialVariableReplacePath(var Path: String): Boolean;
  ...
  begin
    Result := StringWReplace(Path, Variable, VarPath, [rfReplaceAll, rfIgnoreCase]); 
  end;
...
begin
  ...
  begin
    ...
    if Assigned(Desktop) then
      Desktop.ParseDisplayName(ParentWindowHandle, nil, PChar(APath), pchEaten, Result, dwAttributes);
...
begin
  ...
  begin
    ...
    ItemInfoW.dwTypeData := PWideChar( String(Caption));
    ...
    begin
      ...
      begin
        ...
        if not( ItemInfoW.fType and MFT_SEPARATOR <> 0) then
          ItemInfoW.dwTypeData := PWideChar( String(PopupMenu.Items[i].Caption));
