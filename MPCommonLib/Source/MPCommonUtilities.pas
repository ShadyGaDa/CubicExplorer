...
type
  ...
  TCommonStringWDynArray = array of String;
...
function AddCommas(NumberString: String): String;
...
procedure CopyToNullBufferA(S: String; Buffer: PAnsiChar; CharCount: Cardinal);
procedure CopyToNullBufferW(S: String; Buffer: PWideChar; CharCount: Cardinal);
...
function DropEffectToStr(DropEffect: DWORD): String;
function EqualWndMethod(A, B: TWndMethod): Boolean;
function FileExistsW(const FileName: String): Boolean;
function FileIconInit(FullInit: BOOL): BOOL; stdcall;
function FindRootToken(const Path: String): PWideChar;
function FindUniqueMenuID(AMenu: HMenu): Cardinal;
function GetMyDocumentsVirtualFolder: PItemIDList;
function HasMMX: Boolean;
function IncludeTrailingBackslashW(const S: String): String;
function IsAnyMouseButtonDown: Boolean;
function IsFTPPath(Path: String): Boolean;
...
function VariantToCaption(const V: Variant): String;
function WideCreateDir(Path: String): Boolean;
function WideDeleteDirEx(const DirName: String): Boolean;
function WideDirectoryExists(const Name: String): Boolean;
function WideExcludeTrailingBackslash(Path: String): String;
...
function WideExtractFileDir(Path: String): String;
...
function WideExtractFilePath(Path: String): String;
function WideFileExists(Path: String): Boolean;
function WideFindFirstFileEx(FileName: String; var lpFindFileData: TWIN32FindDataW; Mask: String; CaseSensitive: Boolean): THandle;
function WideFindFirstFileExExists: Boolean;
function WideForceDirectories(const FileName: String): Boolean;
function WideGetCurrentDir: String;
function WideGetTempDir: String;
function WideIncludeTrailingBackslash(Path: String): String;
function WideIncrementalSearch(CompareStr, Mask: String): Integer;
function WideIntToStr(Value: integer): String;
...
function WideLowerCase(const Str: String): String;
function WideMessageBox(Window: HWND; const ACaption, AMessage: String; uType: integer): integer;
...
function StringWReplace(const S, OldPattern, NewPattern: String; Flags: TReplaceFlags; WholeWord: Boolean = False): String;
...
procedure LoadStringW(S: TStream; var Str: String);
procedure MinMax(var A, B: Integer);
procedure SaveStringW(S: TStream; Str: String);
function UsesAlphaChannel(Image32: TBitmap): Boolean;
procedure WideInsert(Source: String; var S: String; Index: Integer);
...
// String routines (many borrowed from Mike Liscke)
...
function RegSvrPath: String;
const
  ExeName: String = 'RegSvr32.exe';
var
  Path: String;
begin
  Result:= '';
...
function WideFindFirstFileEx(FileName: String;
  var lpFindFileData: TWIN32FindDataW; Mask: String; CaseSensitive: Boolean): THandle;
var
  CaseFlag: DWORD;
...
begin
  ...
  if Assigned(FindFirstFileExW_MP) then
    Result := FindFirstFileExW_MP(PWideChar(String( FileName + '\' + Mask)), FINDEX_INFO_STANDARD, lpFindFileData, FINDEX_SEARCH_NAMEMATCH, nil, CaseFlag)
  else
...
function ExtractFileDirW(const FileName: String): String;
begin
...
begin
  ...
  try
    ...
    case Delimiter of
      vdeColon:
        begin
          DelimitedText := StringWReplace(DelimitedText, ',', ':', [rfReplaceAll]);
          DelimitedText := StringWReplace(DelimitedText, ';', ':', [rfReplaceAll]);
          DelimitedText := StringWReplace(DelimitedText, '|', ':', [rfReplaceAll]);
  ...
  {    if dtVCenter in Flags then
      ...
      begin
        ...
        begin
          ...
          DrawTextWEx(DC, String(BufferIndex), LineRect, Flags, MaxLineCount);
...
const
  MYCOMPUTER_GUID = String('::{450d8fba-ad25-11d0-98a8-0800361b1103}');
...
procedure WideInsert(Source: String; var S: String; Index: Integer);
{ It appears there is a String Insert in the VCL already but since mine     }
