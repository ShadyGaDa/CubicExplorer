...
type
  ...
  public
    ...
    function AddFolderAndMasks(const FolderInfo: String;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    procedure Clear; override;
    function Matches(const Folder: String; const FindData: TWin32FindDataW): Boolean;
  published
  ...
  protected
    ...
    procedure DoFileMatch(const Folder: String; const FindData: TWin32FindDataW); virtual;
    function IsAcceptable(const Folder: String; const FindData: TWin32FindDataW): Boolean; virtual;
...
function WideExpandUNCFileName(const FileName: String): String;
...
var
  ...
  WideIsDelimiter: array[Char] of Boolean;
...
begin
  ...
  for WC := Low(TCharMap) to High(TCharMap) do
  begin
    CharMap[WC] := WC;
    LowerCharMap[WC] := WC;
    WideIsDelimiter[WC] := not IsCharAlphaNumericW(WC);
  end;
  AnsiLowerBuff(PAnsiChar(@LowerCharMap), SizeOf(LowerCharMap));
end;

{ Helper Functions }

function WideGetUniversalName(const FileName: String): String;
type
...
begin
  ...
  begin
    ...
    if WNetGetUniversalNameW(PChar(FileName), UNIVERSAL_NAME_INFO_LEVEL,
        ...
        begin
          ...
          for I := 0 to Count - 1 do
            with NetResources^[I] do
              if (lpLocalName <> nil) and (Drive = LowerCharMap[lpLocalName[0]]) then
              begin
...
//  Result := WideExpandFileName(FileName);
  ...
  if (Length(Result) >= 3) and (Result[2] = ':') and
     (LowerCharMap[Result[1]] >= 'a') and
     (LowerCharMap[Result[1]] <= 'z')
  then
    Result := WideGetUniversalName(Result);
end;

function DecodeUTF8Char(Buffer: PAnsiChar; Count: Integer): Char;
...
{$IFNDEF UNICODE}
function AnsiStringToString(const S: AnsiString): String;
var
  Len: Integer;
begin
  if S <> '' then
  begin
    ...
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, PAnsiChar(S), -1, PChar(Result), Len);
  end
  else
    Result := '';
end;
...
function StreamContainsPhraseWide(Stream: TStream; const Phrase: PChar;
  PhraseLen: Integer; Options: TContentSearchOptions; Swapped: Boolean): Boolean;
const
  MaxBufferSize = $F000;
var
  CharMap: PCharMap;
  PrvChar, NxtChar: Char;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              NxtChar := bp^;
            if Swapped then
              NxtChar := Char(MakeWord(HiByte(WORD(NxtChar)), LoByte(WORD(NxtChar))));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Result := StreamContainsPhraseWide(Stream, PChar(Phrase.Unicode),
...
function StringListFromString(const Str: String;
...
begin
  Result := Info;
//  P := WideTextPos(IncludeSign, Info);     //tnt
  P := Pos(WideUpperCase(IncludeSign), WideUpperCase(Info));
...
//    P := WideTextPos(ExcludeSign, Info);
    P := Pos(WideUpperCase(ExcludeSign), WideUpperCase(Info));
    if P <> 0 then
    begin
      System.Delete(Result, 1, P);
      Recursive := False;
...
procedure TFindFileW.DoFileMatch(const Folder: String;
...
function TFindFileW.DoFolderChange(const Folder: String): TFolderIgnore;
begin
  Result:= fiNone;
...
function TFindFileW.IsAcceptable(const Folder: String;
  const FindData: TWin32FindDataW): Boolean;
begin
...
procedure TFindFileW.SearchIn(const Path: String);
var
  I: Integer;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          Handle := Windows.FindFirstFileW(PChar(Path + FileMasks[I]), FindData);
      ...
      begin
        Handle := Windows.FindFirstFileW(PChar(Path + '*.*'), FindData);
        if Handle <> INVALID_HANDLE_VALUE then
          try
            repeat
              if Aborted then Exit;
