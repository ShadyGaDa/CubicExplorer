...
{$Q-}
function TSynFortranSyn.HashKey(Str: PChar{!}): Cardinal;
begin
  Result := 0;
...
begin
  if (((SynUpperCase(FLine[Run + 1]) = 'G') and CharInSet(SynUpperCase(FLine[Run + 2])[1], ['E', 'T'])) {.ge. .gt.}
       or ((SynUpperCase(FLine[Run + 1]) = 'L') and CharInSet(SynUpperCase(FLine[Run + 2])[1], ['E', 'T'])) {.le. .lt.}
       or ((SynUpperCase(FLine[Run + 1]) = 'N') and (SynUpperCase(FLine[Run + 2]) = 'E')) {.ne.}
       or ((SynUpperCase(FLine[Run + 1]) = 'E') and (SynUpperCase(FLine[Run + 2]) = 'Q')) {.eq.}
       or ((SynUpperCase(FLine[Run + 1]) = 'O') and (SynUpperCase(FLine[Run + 2]) = 'R'))){.or.}
     and (FLine[Run + 3] = '.') then
    begin
      Inc(Run, 4);
      FTokenID := tkSymbol;
    end
  else if (((SynUpperCase(FLine[Run + 1]) = 'A')
              and (SynUpperCase(FLine[Run + 2]) = 'N')
              and (SynUpperCase(FLine[Run + 3]) = 'D'))    {.and.}
           or ((SynUpperCase(FLine[Run + 1]) = 'N')
              and (SynUpperCase(FLine[Run + 2]) = 'O')
              and (SynUpperCase(FLine[Run + 3]) = 'T')))    {.not.}
          and (FLine[Run + 4] = '.') then
    begin
      Inc(Run, 5);
      FTokenID := tkSymbol;
    end
  else if (SynUpperCase(FLine[Run + 1]) = 'T')
          and (SynUpperCase(FLine[Run + 2]) = 'R')
          and (SynUpperCase(FLine[Run + 3]) = 'U')
          and (SynUpperCase(FLine[Run + 4]) = 'E')
          and (FLine[Run + 5] = '.') then  {.true.}
    begin
      Inc(Run, 6);
      FTokenID := tkSymbol;
    end
  else if (SynUpperCase(FLine[Run + 1]) = 'F')
          and (SynUpperCase(FLine[Run + 2]) = 'A')
          and (SynUpperCase(FLine[Run + 3]) = 'L')
          and (SynUpperCase(FLine[Run + 4]) = 'S')
          and (SynUpperCase(FLine[Run + 5]) = 'E')
