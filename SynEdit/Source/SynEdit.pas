...
{$ENDIF}
  ...
  TDropFilesEvent = procedure(Sender: TObject; X, Y: Integer; AFiles: TuStrings)
    of object;
...
{$ENDIF}
    ...
    procedure SetLines(Value: TuStrings);
...
{$ENDIF}
  ...
  public
    ...
    property Lines: TuStrings read FLines write SetLines;
...
function TCustomSynEdit.GetSelText: String{!};
  ...
  begin
    ...
    if SrcLen >= DstLen then
      ...
      P := PChar{!}(Result);
      WStrCopy(P, PChar{!}(Copy(S, Index, Count)));
      Inc(P, Length(S));
...
begin
  if WantTabs and not(eoTabsToSpaces in Options) and (CharCount >= TabWidth) then
    Result := uStringOfChar(#9, CharCount div TabWidth) +
      uStringOfChar(#32, CharCount mod TabWidth)
  else
    Result := uStringOfChar(#32, CharCount);
end;
...
{$IFDEF SYN_CLX}
        ...
        if FGutter.Gradient then
          Canvas.Brush.Style := bsSolid;
{$ELSE}
        TextSize := GetTextSize(DC, PChar{!}(s), Length(s));
...
begin
  ...
  begin
    ...
    DWriteFactory.CreateTextFormat(PChar{!}(CurrentFont.Name), nil,
    ...
    begin
      ...
      OleCheck(DWriteFactory.CreateTextLayout(PChar{!}(Str), Length(Str),
...
begin
  ...
  DWriteFactory.CreateTextFormat(PChar{!}(Self.Font.Name), nil,
...
{ local procedures }
  ...
  // like empty token rect or invalid indices into TokenLen.
  // CharsBefore tells if Token starts at column one or not
  procedure PaintTokenD2D(Token: String{!};
    TokenLen, CharsBefore, First, Last: Integer);
  var
    Text: String{!};
    Counter, nX, nCharsToPaint: Integer;
  ...
  begin
    ...
    begin
      ...
      OleCheck(DWriteFactory.CreateTextLayout(PChar{!}(Text), nCharsToPaint,
      ...
      begin
        ...
        begin
          ...
          OleCheck(DWriteFactory.CreateTextLayout(PChar{!}(Text), 1,
            TextFormat, rcToken.Width, rcToken.Height, TextLayout));
  ...
  // record. This will paint any chars already stored if there is
  // a (visible) change in the attributes.
  procedure AddHighlightTokenD2D(const Token: String{!};
    CharsBefore, TokenLen: Integer;
    Foreground, Background: TColor;
    Style: TFontStyles);
  var
...
{$ENDIF}
    ...
    SetSelTextPrimitiveEx(PasteMode, PChar{!}(GetClipboardText), True);
    vEndOfBlock := BlockEnd;
...
procedure TCustomSynEdit.SetLines(Value: TuStrings);
begin
  Lines.Assign(Value);
end;
...
var
  ...
  procedure InsertText;
    ...
    begin
      ...
      begin
        sLeftSide := sLeftSide + uStringOfChar(#32,
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          begin
            Inc(Result);
            TempString := uStringOfChar(#32, InsertPos - 1) + Str;
            Lines.Add('');
            ...
            begin
              TempString :=
                TempString + uStringOfChar(#32, InsertPos - Len - 1) + Str
            end
            else
...
var
  ...
  Point: TPoint;
  FilesList: TuStringList;
begin
  try
    if Assigned(FOnDropFiles) then
    begin
      FilesList := TuStringList.Create;
      try
...
procedure TCustomSynEdit.InsertLine(const BB, BE: TBufferCoord; ChangeStr: PChar{!};
  AddToUndoList: Boolean);
begin
  SetCaretAndSelection(BB, BB, BE);
      ...
      crLineBreak:
        begin
          ...
          begin
            ...
            then
              TmpStr := TmpStr + uStringOfChar(#32, CaretX - 1 - Length(TmpStr));
        ...
        if not ReadOnly then begin
          ...
          if SelAvail then
            ...
            begin
              ...
              if CaretY < Lines.Count then
              begin
                Helper := uStringOfChar(#32, CaretX - 1 - Len);
      ...
      ecDeleteWord, ecDeleteEOL:
        if not ReadOnly then begin
          ...
          begin
            ...
            Helper := SelText;
            SetSelTextPrimitive(uStringOfChar(' ', CaretX - BlockBegin.Char));
      ...
      // #127 is Ctrl + Backspace, #32 is space
        ...
        begin
          ...
          begin
            ...
            begin
              if (Len > 0) then
                SpaceBuffer := uStringOfChar(#32, CaretX - Len - Ord(FInserting))
              else
...
// Processing of case character covers on LeadByte.
      ...
      ecImeStr:
        if not ReadOnly then
        begin
          SetString(S, PChar{!}(Data), WStrLen(Data));
          if SelAvail then
          begin
            BeginUndoBlock;
            try
          ...
          begin
            ...
            Len := Length(Temp);
            if Len < CaretX then
              Temp := Temp + uStringOfChar(#32, CaretX - Len - 1);
            bChangeScroll := not (eoScrollPastEol in FOptions);
            try
              if bChangeScroll then Include(FOptions, eoScrollPastEol);
              StartOfBlock := CaretXY;
...
{$IFDEF SYN_DirectWrite}
      ...
      begin
        ...
        DWriteFactory.CreateTextFormat(PChar{!}(FGutter.Font.Name), nil,
        ...
        FontCollection.FindFamilyName(PChar{!}(Self.Font.Name),
        ...
        begin
          OleCheck(DWriteFactory.CreateTextLayout(PChar{!}('M'), 1, TextFormat,
            0, 0, TextLayout));
...
{$IFDEF SYN_DirectWrite}
  ...
  begin
    ...
    FontCollection.FindFamilyName(PChar{!}(Self.Font.Name), FontIndex, FontExists);
    if FontExists then
    begin
      FontCollection.GetFontFamily(FontIndex, FontFamily);
...
procedure TCustomSynEdit.DoCaseChange(const Cmd: TSynEditorCommand);
  ...
  var
    ...
    sLower: String{!};
  begin
    Result := SynUpperCase(aStr);
    sLower := SynLowerCase(aStr);
  ...
  var
    i: Integer;
  begin
   Result := SynLowerCase(aStr);
   for i := 1 to Length(Result) do
   if (i = 1) or IsWordBreakChar(Result[i-1]) then Result[i] := SynUpperCase(Result[i])[1];
  end;
      ...
      begin
        ...
        begin
          ...
          if nDistanceToTab > 0 then
            Spaces := Spaces + uStringOfChar(#32, nDistanceToTab);
        end else
          Spaces := #9;
      end
      else begin
...
begin
  ...
  try
    ...
    begin
      ...
      Run := StrToInsert;
      Spaces := uStringOfChar(#32, FTabWidth);
    end
    else begin
    ...
    begin
      WStrCopy(Run, PChar{!}(Spaces + #13#10));
      Inc(Run, Length(spaces) + 2);
    end;
    WStrCopy(Run, PChar{!}(Spaces));
...
begin
  ...
  begin
    ...
    begin
       ...
       //Instead of doing a uStringOfChar, we need to get *exactly* what was
       //being deleted incase there is a TabChar
       TmpDelLen := GetDelLen;
       WStrCat(StrToDelete, PChar{!}(Copy(Line, 1, TmpDelLen)));
       WStrCat(StrToDelete, PChar{!}(String{!}(#13#10)));
...
{$IFNDEF SYN_COMPILER_17_UP}
  function Low(AStr: String{!}): Integer; {$IFDEF SYN_COMPILER_9_UP}inline;{$ENDIF}
  begin
    Result := 1;
  end;

  function High(AStr: String{!}): Integer; {$IFDEF SYN_COMPILER_9_UP}inline;{$ENDIF}
  begin
    Result := Length(AStr);
  end;
{$ENDIF}
