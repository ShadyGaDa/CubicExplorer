...
function TSynCobolSyn.HashKey(Str: PChar{!}): Integer;
var
  InternalRun: LongInt;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            while FLine[I] = ' ' do
              Inc(I);
            if (WStrLComp(PChar{!}(@FLine[I]), 'record', Length('record')) = 0)
    ...
    case FLine[Run] of
      ...
      '-': if FRange in [rsQuoteStringMayBe, rsApostStringMayBe] then
           begin
             ...
             while FLine[I] = ' ' do
               Inc(I);
             if (WStrLComp(PChar{!}(@FLine[I]), PChar{!}(uStringOfChar(StringChars[FRange], 2)), 2) <> 0)
...
function TSynCobolSyn.IsIdentChar(AChar: Char{!}): Boolean;
begin
  case AChar of
    '-', '0'..'9', 'a'..'z', 'A'..'Z':
      Result := True;
