...
type
{$IFDEF USE_TNT_DESIGNTIME_SUPPORT}
  // Wrapper around TuStringListProperty to enable the TNT property editor to
  // handle TuStrings
  TSynuStringListProperty = class(TWideStringListProperty)
  private
    FuStrings: TuStrings;
...
{ TSynuStringListProperty }

{$IFDEF SYN_COMPILER_6_UP}
constructor TSynuStringListProperty.Create(const ADesigner: IDesigner; APropCount: Integer);
{$ELSE}
constructor TSynuStringListProperty.Create(const ADesigner: IFormDesigner; APropCount: Integer);
{$ENDIF}
begin
  inherited;
  FuStrings := TuStringList.Create;
  FTntStrings := TTntStringList.Create;
end;

destructor TSynuStringListProperty.Destroy;
begin
  FTntStrings.Free;
  FuStrings.Free;
  inherited;
...
function TSynuStringListProperty.GetStrings: TTntStrings;
var
  uStrings: TuStrings;
  i: Integer;
begin
  uStrings := TuStrings(GetOrdValue);

  FTntStrings.Clear;
  FTntStrings.BeginUpdate;
  try
    for i := 0 to uStrings.Count - 1 do
      FTntStrings.AddObject(uStrings[i], uStrings.Objects[i]);
  finally
...
procedure TSynuStringListProperty.SetStrings(const Value: TTntStrings);
var
  i: Integer;
begin
  FuStrings.Clear;
  FuStrings.BeginUpdate;
  try
    for i := 0 to Value.Count - 1 do
      FuStrings.AddObject(Value[I], Value.Objects[I]);
  finally
    FuStrings.EndUpdate;
  end;
  SetOrdValue(Longint(FuStrings));
end;
{$ENDIF}
...
{$IFDEF USE_TNT_DESIGNTIME_SUPPORT}
  // Troy Wolbrink added my (Maël Hörz) Char{!} property editor to
  // TntuStringProperty_Design.pas.
  // As it is registered there, no need to do it a second time here.
  // However as he uses TTntStrings and we use TuStrings, we need
  // a wrapper to do the "translation".
  RegisterPropertyEditor(TypeInfo(TuStrings), nil,
     '', TSynuStringListProperty);
{$ELSE}
  RegisterPropertyEditor(TypeInfo(Char), nil,
     '', TCharProperty);
  RegisterPropertyEditor(TypeInfo(TuStrings), nil,
     '', TStringListProperty);
{$ENDIF}
