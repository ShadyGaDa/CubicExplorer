...
type
  ...
  public
    ...
    constructor Create(AName: string; AFriendlyName: string{!}); overload;
    procedure InternalSaveDefaultValues;
{$IFNDEF SYN_CLX}
    function LoadFromBorlandRegistry(RootKey: HKEY; AttrKey, AttrName: string; OldStyle: Boolean): Boolean; virtual;
...
{$ENDIF}
  ...
  TSynHighlighterCapability = (hcUserSettings, // supports Enum/UseUserSettings
...
type
  ...
  protected
    ...
    function GetDefaultAttribute(Index: Integer): TSynHighlighterAttributes; virtual; abstract;
  ...
  public
    ...
    procedure SetLineExpandedAtWideGlyphs(const Line, ExpandedLine: string{!}; LineNumber: Integer); virtual;
...
{$ENDIF}
  ...
  public
    ...
    property Attribute[Index: Integer]: TSynHighlighterAttributes read GetAttribute;
    ...
    property CommentAttribute: TSynHighlighterAttributes index SYN_ATTR_COMMENT read GetDefaultAttribute;
    property IdentifierAttribute: TSynHighlighterAttributes index SYN_ATTR_IDENTIFIER read GetDefaultAttribute;
    property KeywordAttribute: TSynHighlighterAttributes index SYN_ATTR_KEYWORD read GetDefaultAttribute;
    property StringAttribute: TSynHighlighterAttributes index SYN_ATTR_STRING read GetDefaultAttribute;
    property SymbolAttribute: TSynHighlighterAttributes index SYN_ATTR_SYMBOL read GetDefaultAttribute;
    property WhitespaceAttribute: TSynHighlighterAttributes index SYN_ATTR_WHITESPACE read GetDefaultAttribute;
    property ExportName: string read GetExportName;
  published
    property DefaultFilter: string read GetDefaultFilter write SetDefaultFilter stored IsFilterStored;
...
procedure TSynCustomHighlighter.DoSetLine(const Value: String{!}; LineNumber: Integer);
  ...
  begin
    ...
    dest := SynLowerCase(value);
  end;

begin
  // uStrings are not reference counted, hence we need to copy
...
// Pos and Result are 1-based (i.e. positions in a uString not a PChar{!})
