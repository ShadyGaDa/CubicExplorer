...
CONTENTS:
  ...
  THeaderFooter:
    ...
    Run-time methods:
      function Add(Text: uString; Font: TFont;
      ...
      procedure SetPixPrInch(Value : Integer);
        ...
        TSynEditPrint.
      procedure InitPrint(ACanvas : TCanvas;NumPages : Integer; Title : uString;
...
var
  ...
  begin
    Result := True;
    Macro := SynUpperCase(Copy(FText, Start, Run - Start + 1));
...
begin
  ...
  begin
    ...
    try
      ...
      PChar{!}(Buffer)[BufferSize div sizeof(Char{!})] := #0;
      FText := PChar{!}(Buffer);
    finally
      FreeMem(Buffer);
    end;
...
begin
  ...
  begin
    ...
    Write(PChar{!}(FText)^, aLen * sizeof(Char{!}));
...
begin
  ...
  {$IFNDEF SYN_CLX}
    ...
    ExtTextOutW(ACanvas.Handle, X, Y + TLineInfo(FLineInfo[CurLine - 1]).MaxBaseDist,
      0, nil, PChar{!}(AStr), Length(AStr), nil);
