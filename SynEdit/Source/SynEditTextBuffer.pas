...
type
  ...
  TSynEditStringRec = record
    {$IFDEF OWN_uString_MEMMGR}
    FString: PChar{!}; // "array of Char{!}";
    {$ELSE}
    FString: String{!};
    {$ENDIF OWN_uString_MEMMGR}
    FObject: TObject;
...
type
  ...
  TSynEditStringList = class(TuStrings)
  private
  ...
  public
    ...
    procedure AddStrings(Strings: TuStrings); override;
    ...
    procedure InsertStrings(Index: Integer; NewStrings: TuStrings);
...
procedure TSynEditStringList.AddStrings(Strings: TuStrings);
var
...
procedure TSynEditStringList.InsertText(Index: Integer;
  NewText: String{!});
var
  TmpStringList: TuStringList;
begin
  if NewText = '' then Exit;

  TmpStringList := TuStringList.Create;
  try
