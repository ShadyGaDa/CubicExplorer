...
{$ENDIF}
    procedure SetSecondKeys(const Value: TuStrings);
  protected
  ...
  published
    ...
    property SecondKeyWords: TuStrings read FSecondKeys write SetSecondKeys;
...
begin
  ...
  FSecondKeys := TuStringList.Create;
  TuStringList(FSecondKeys).Sorted := True;
  TuStringList(FSecondKeys).Duplicates := dupIgnore;
...
{$IFDEF SYN_HEREDOC}
  ...
  begin
    ...
    if (FLine[Run + 2] in [Char{!}('-'), Char{!}('"'), Char{!}(''''), Char{!}('`')]) then
    begin
      SkipRun := 1;
      if FLine[Run + 2] <> '-' then
...
begin
  ...
  if FRange = rsIndentedHeredoc then
    while FLine[Run] in [Char{!}(#9), Char{!}(#32)] do Inc(Run);
...
procedure TSynRubySyn.SetSecondKeys(const Value: TuStrings);
var
  i: Integer;
begin
  if Value <> nil then
