...
type
  ...
  protected
    ...
    function ReplaceReservedChar(AChar: Char{!}): String{!}; virtual; abstract;
  ...
  public
    ...
    procedure ExportAll(ALines: TuStrings);
    { Exports the given range of the strings parameter to the output buffer. }
    procedure ExportRange(ALines: TuStrings; Start, Stop: TBufferCoord);
  ...
  try
    ...
    begin
      ...
      try
        if P <> nil then
        begin
          Move(PChar{!}(Text)^, P^, (SLen + 1) * sizeof(Char{!}));
...
begin
  ...
  begin
    ...
    case Encoding of
      seUTF16LE:
        S := PChar{!}(FBuffer.Memory);
      seUTF16BE:
        begin
          S := PChar{!}(FBuffer.Memory);
          StrSwapByteOrder(PChar{!}(S));
        end;
      seUTF8:
{$IFDEF UNICODE}
...
procedure TSynCustomExporter.ExportAll(ALines: TuStrings);
begin
  ExportRange(ALines, BufferCoord(1, 1), BufferCoord(MaxInt, MaxInt));
end;

procedure TSynCustomExporter.ExportRange(ALines: TuStrings; Start, Stop: TBufferCoord);
var
  i: Integer;
  Line, Token: String{!};
...
begin
  ...
  case Encoding of
    ...
    seUTF16LE, seUTF16BE:
      Result := Length(AText);
    seAnsi:
      Result := Length(AnsiString(PChar{!}(AText)));
  end;
...
begin
  case Encoding of
    ...
    seUTF16LE:
      FBuffer.WriteBuffer(AText[1], Length(AText) * sizeof(Char{!}));
    seUTF16BE:
      begin
        StrSwapByteOrder(PChar{!}(AText));
        FBuffer.WriteBuffer(AText[1], Length(AText) * sizeof(Char{!}));
      end;
    seAnsi:
      begin
        AnsiStr := AnsiString(PChar{!}(AText));
