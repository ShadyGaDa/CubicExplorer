...
type
  ...
  private
    ...
    procedure SetLines(const Value: TuStrings);
...
procedure TSynEditPrint.SetLines(const Value: TuStrings);
var
  i, j: Integer;
...
//  fTestString := uStringOfChar('W', FMaxCol);
  AStr := uStringOfChar('W', FMaxCol);
...
var
  ...
  begin
    ...
    Windows.ExtTextOutW(FCanvas.Handle, X, Y, 0, nil, PChar{!}(Text),
...
begin
  ...
  begin
    Read(Len, sizeof(Len));
    BufferSize := Len * sizeof(Char{!});
    GetMem(Buffer, BufferSize + sizeof(Char{!}));
    try
      Read(Buffer^, BufferSize);
      Buffer[BufferSize div sizeof(Char{!})] := #0;
      FTitle := Buffer;
    finally
      FreeMem(Buffer);
    end;
...
begin
  ...
  begin
    ...
    Write(PChar{!}(FTitle)^, aLen * sizeof(Char{!}));
    aLen := Length(FDocTitle);
    Write(aLen, SizeOf(aLen));
    Write(PChar{!}(FDocTitle)^, aLen * sizeof(Char{!}));
