...
type
  ...
  private
    ...
    procedure SetKeyWords(const Value: TuStrings);
  ...
  published
    ...
    property IdentifierChars: String{!} read GetIdentifierChars write SetIdentifierChars stored StoreIdentChars;
    property KeyAttri: TSynHighlighterAttributes read FKeyAttri write FKeyAttri;
    property KeyWords: TuStrings read FKeyWords write SetKeyWords;
...
begin
  ...
  FKeyWords := TuStringList.Create;
  TuStringList(FKeyWords).Sorted := True;
  TuStringList(FKeyWords).Duplicates := dupIgnore;
...
function TSynGeneralSyn.IsIdentChar(AChar: Char{!}): Boolean;
var
  i: Integer;
begin
  Result := False;
...
function TSynGeneralSyn.IsWordBreakChar(AChar: Char{!}): Boolean;
begin
...
procedure TSynGeneralSyn.SetKeyWords(const Value: TuStrings);
var
  i: Integer;
begin
  if Value <> nil then
  begin
    ...
    for i := 0 to Value.Count - 1 do
      Value[i] := SynUpperCase(Value[i]);
    Value.EndUpdate;
  end;

  TuStringList(FKeyWords).Sorted := False;
