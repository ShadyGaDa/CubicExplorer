...
begin
  ...
  FLookAt := 1;
  for C := Low(Char{!}) to High(Char{!}) do FShift[C] := FPatLenSucc;
...
function IsWordBreakChar(C: Char{!}): Boolean;
begin
  case C of
...
begin
  ...
  begin
    if CaseSensitive then
      ...
      FTextToSearch := SynLowerCase(NewText);
