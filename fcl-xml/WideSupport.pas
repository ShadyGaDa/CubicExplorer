...
function WideCreateFile(lpFileName: PAnsiChar; dwDesiredAccess, dwShareMode:
...
function WideFileOpen(const FileName: AnsiString; Mode: LongWord): Integer;
...
begin
  Result := Integer(WideCreateFile(PAnsiChar(FileName), AccessMode[Mode and 3],
    ShareMode[(Mode and $F0) shr 4], nil, OPEN_EXISTING,
      FILE_ATTRIBUTE_NORMAL, 0));
end;

constructor TWideFileStream.Create(const FileName: AnsiString; Mode: Word);
var
  CreateHandle: Integer;
begin
  if Mode = fmCreate then
  begin
    CreateHandle := Integer(WideCreateFile(PAnsiChar(FileName), GENERIC_READ or GENERIC_WRITE,
