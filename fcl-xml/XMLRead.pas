...
procedure ReadXMLFile(out ADoc: TXMLDocument; const AFilename: AnsiString);
    overload;
...
procedure ReadXMLFragment(AParentNode: TDOMNode; var f: TStream; const
    ABaseURI: AnsiString); overload;

procedure ReadDTDFile(out ADoc: TXMLDocument; const AFilename: AnsiString);
    overload;
...
//    property BaseURI: AnsiString read FBaseURI write FBaseURI;
...
//    property Encoding: string read FEncoding write FEncoding;
  ...
  public
    ...
    procedure ParseUri(const URI: AnsiString; out ADoc: TXMLDocument);
...
type
  ...
  private
    ...
    FSystemID: AnsiString;
    FPublicID: AnsiString;
    function GetSystemID: AnsiString;
    function GetPublicID: AnsiString;
  public
    constructor Create(const AData: AnsiString);
    function NextChar: AnsiChar; virtual;
    procedure Initialize; virtual;
    function SetEncoding(const AEncoding: AnsiString): Boolean; virtual;
    property SystemID: AnsiString read GetSystemID write FSystemID;
    property PublicID: AnsiString read GetPublicID write FPublicID;
  end;
  ...
  private
    FDecoder: TDecoder;
    FSeenCR: Boolean;
    function InternalNextChar: AnsiChar;
    procedure DecodingError(const Msg: AnsiString);
  protected
    procedure FetchData; virtual;
  public
    destructor Destroy; override;
    function NextChar: AnsiChar; override;
    function SetEncoding(const AEncoding: AnsiString): Boolean; override;
  ...
  public
    ...
    function DecodeNext: AnsiChar; virtual; abstract;
    class function Supports(const AEncoding: AnsiString): Boolean; virtual; abstract;
  end;

  TISO8859_1Decoder = class(TDecoder)
  public
    function DecodeNext: AnsiChar; override;
    class function Supports(const AEncoding: AnsiString): Boolean; override;
  end;
  ...
  private
    ...
    procedure DoParseAttValue(Delim: AnsiChar);
    ...
    function ValidateAttrSyntax(AttrDef: TDOMAttrDef; const aValue: AnsiString): Boolean;
    procedure AddIdRef(Buf: PAnsiChar; Length: Integer);
  ...
  protected
    ...
    procedure FatalError(const descr: AnsiString; AtTokenStart: Boolean=False); overload;
    procedure FatalError(const descr: AnsiString; const args: array of const; AtTokenStart: Boolean=False); overload;
    procedure FatalError(Expected: AnsiChar); overload;
    function  SkipWhitespace: Boolean;
    function  SkipWhitespaceRaw: Boolean;
    procedure ExpectWhitespace;
    procedure ExpectString(const s: AnsiString);
    procedure ExpectChar(wc: AnsiChar);
    function  CheckForChar(c: AnsiChar): Boolean;
    procedure SkipString(const ValidChars: TSetOfChar);
    function  GetString(const ValidChars: TSetOfChar): AnsiString;

    procedure RaiseNameNotFound;
    function  CheckName: Boolean;
    function  CheckNmToken: Boolean;
    function  ExpectName: AnsiString;                                   // [5]
    procedure SkipName;
    function SkipQuotedLiteral: Boolean;
    procedure ExpectAttValue;                                           // [10]
    procedure SkipPubidLiteral;                                         // [12]
    procedure SkipSystemLiteral(out Literal: AnsiString);
    ...
    function  ResolvePredefined(const RefName: AnsiString): AnsiChar;
    ...
    function  ParseExternalID(out SysID, PubID: AnsiString;             // [75]
      SysIdOptional: Boolean): Boolean;
    procedure ProcessTextAndRefs;

    procedure BadPENesting;
    procedure ParseEntityDecl;
    procedure ParseEntityDeclValue(Delim: AnsiChar);
    procedure ParseAttlistDecl;
    procedure ExpectChoiceOrSeq(CP: TContentParticle);
    procedure ParseElementDecl;
    procedure ParseNotationDecl;
    function ResolveEntity(const SystemID, PublicID: AnsiString; out Source: TXMLCharSource): Boolean;
    ...
    procedure ValidationError(const Msg: AnsiString; const args: array of const);
    procedure CheckNotation(const Name: AnsiString);
    // Some SAX-alike stuff (at a very early stage)
    procedure DoText(ch: PAnsiChar; Count: Integer; Whitespace: Boolean=False);
    procedure DoComment(ch: PAnsiChar; Count: Integer);
    procedure DoCDSect(ch: PAnsiChar; Count: Integer);
    procedure DoNotationDecl(const aName, aPubID, aSysID: AnsiString);
  public
    doc: TDOMDocument;
  ...
  protected
    ...
    FEnumeration: array of AnsiString;
    function AddEnumToken(const aValue: AnsiString): Boolean;
    function HasEnumToken(const aValue: AnsiString): Boolean;
  end;
...
function FindDecoder(const Encoding: AnsiString): TDecoderRef;
begin
...
begin
  ...
  ABuffer.Length := 0;
  ABuffer.Buffer := AllocMem(ABuffer.MaxLength*SizeOf(AnsiChar));
end;

procedure BufAppend(var ABuffer: TWideCharBuf; wc: AnsiChar);
var
  OldLength: Integer;
begin
  ...
  begin
    ...
    ReallocMem(ABuffer.Buffer, ABuffer.MaxLength * SizeOf(AnsiChar));
    FillChar(ABuffer.Buffer[OldLength],(ABuffer.MaxLength-OldLength) * SizeOf(AnsiChar),0);
  end;
...
procedure TDOMParser.ParseUri(const URI: AnsiString; out ADoc: TXMLDocument);
var
  Src: TXMLCharSource;
begin
  ADoc := nil;
...
constructor TXMLCharSource.Create(const AData: AnsiString);
begin
  inherited Create;
//  FBuf := PChar(PAnsiChar(AData));
  FBuf :=PAnsiChar(AData);
  FBufEnd := FBuf + Length(AData) * sizeof(AnsiChar);
end;
...
function TXMLCharSource.NextChar: AnsiChar;
begin
  Result := FSurrogate;
  FSurrogate := #0;
  if Result <> #0 then
  ...
  begin
    Result := PAnsiChar(FBuf)^;
    Inc(FBuf, sizeof(AnsiChar));
  end;
end;

function TXMLCharSource.SetEncoding(const AEncoding: AnsiString): Boolean;
begin
  Result := True; // always succeed
end;

function TXMLCharSource.GetPublicID: AnsiString;
begin
  if FPublicID <> '' then
    Result := FPublicID
...
function TXMLCharSource.GetSystemID: AnsiString;
begin
  if FSystemID <> '' then
    Result := FSystemID
  else if Assigned(FParent) then
...
procedure TXMLDecodingSource.DecodingError(const Msg: AnsiString);
begin
...
function TXMLDecodingSource.InternalNextChar: AnsiChar;
begin
...
function TXMLDecodingSource.NextChar: AnsiChar;
begin
  ...
  if (Result < #256) and (AnsiChar(Result) in TXMLReader(FReader).FForbiddenAscii) or
    (Result >= #$FFFE) then
...
function TXMLDecodingSource.SetEncoding(const AEncoding: AnsiString): Boolean;
var
...
function TISO8859_1Decoder.DecodeNext: AnsiChar;
begin
  with FSource do
  begin
    Result := AnsiChar(FBuf[0]);
    Inc(FBuf);
  end;
end;

class function TISO8859_1Decoder.Supports(const AEncoding: AnsiString): Boolean;
begin
...
function TUCS2Decoder.DecodeNext: AnsiChar;
begin
  with FSource do
  begin
    Result := PAnsiChar(FBuf)^;
  ...
  if FSwapEndian then
    Result := AnsiChar(Swap(Word(Result)));
end;

class function TUCS2Decoder.Supports(const AEncoding: AnsiString): Boolean;
begin
...
function TUTF8Decoder.DecodeNext: AnsiChar;
const
...
begin
  ...
  begin
    ...
    case Value of
      ...
      $10000..$10FFFF:
        begin
          Result := AnsiChar($D7C0 + (Value shr 10));
          FSurrogate := AnsiChar($DC00 xor (Value and $3FF));
          Exit;
        end;
    end;
    DecodingError('UCS4 character out of supported range');
...
class function TUTF8Decoder.Supports(const AEncoding: AnsiString): Boolean;
begin
...
function TXMLReader.ResolveEntity(const SystemID, PublicID: AnsiString; out Source: TXMLCharSource): Boolean;
var
  AbsSysID: AnsiString;
  Filename: AnsiString;
  Stream: TStream;
...
procedure TXMLReader.FatalError(Expected: AnsiChar);
begin
// FIX: don't output what is found - anything may be found, including exploits...
  FatalError('Expected "%1s"', [AnsiString(Expected)]);
end;

procedure TXMLReader.FatalError(const descr: AnsiString; AtTokenStart: Boolean);
var
...
procedure TXMLReader.FatalError(const descr: AnsiString; const args: array of const; AtTokenStart: Boolean);
begin
...
procedure TXMLReader.ExpectChar(wc: AnsiChar);
begin
  if FCurChar = wc then
    GetChar
  else
...
begin
  FValue.Length := 0;
  while (ord(FCurChar) < 256) and (AnsiChar(FCurChar) in ValidChars) do
  begin
...
function TXMLReader.ResolvePredefined(const RefName: AnsiString): AnsiChar;
begin
  if RefName = 'amp' then
    Result := '&'
...
begin
  ...
  begin
    ...
    case Value of
      $01..$08, $0B..$0C, $0E..$1F:
        if FXML11 then
          BufAppend(FValue, AnsiChar(Value))
        else
      ...
      $10000..$10FFFF:
        begin
          BufAppend(FValue, AnsiChar($D7C0 + (Value shr 10)));
          BufAppend(FValue, AnsiChar($DC00 xor (Value and $3FF)));
        end;
    else
      FatalError('Invalid character reference');
    end;
...
procedure TXMLReader.DoParseAttValue(Delim: AnsiChar);
var
...
begin
  ...
  begin
    ...
    begin
      wc := FValue.Buffer[I];
      if (wc > #255) or not (AnsiChar(wc) in PubidChars) then
...
procedure TXMLReader.SkipSystemLiteral(out Literal: AnsiString);
begin
  if SkipQuotedLiteral then
...
procedure TXMLReader.CheckNotation(const Name: AnsiString);
begin
...
procedure TXMLReader.ParseEntityDeclValue(Delim: AnsiChar);   // [9]
var
  I: Integer;
  Src: TXMLCharSource;
begin
...
procedure TXMLReader.AddIdRef(Buf: PAnsiChar; Length: Integer);
var
  w: PWideStrWrapper;
begin
  New(w);
...
function TXMLReader.ParseExternalID(out SysID, PubID: AnsiString;     // [75]
  SysIdOptional: Boolean): Boolean;
begin
  if FCurChar = 'S' then
  begin
...
procedure TXMLReader.ValidationError(const Msg: AnsiString; const Args: array of const);
var
  E: EXMLReadError;
begin
...
function TXMLReader.ValidateAttrSyntax(AttrDef: TDOMAttrDef; const aValue: AnsiString): Boolean;
begin
...
procedure TXMLReader.DoText(ch: PAnsiChar; Count: Integer; Whitespace: Boolean);
var
  TextNode: TDOMText;
begin
...
procedure TXMLReader.DoComment(ch: PAnsiChar; Count: Integer);
var
  ElDef: TDOMElementDef;
begin
...
procedure TXMLReader.DoCDSect(ch: PAnsiChar; Count: Integer);
var
  s: AnsiString;
  ElDef: TDOMElementDef;
begin
...
procedure TXMLReader.DoNotationDecl(const aName, aPubID, aSysID: AnsiString);
var
...
function TDOMAttrDef.AddEnumToken(const aValue: AnsiString): Boolean;
var
  I, L: Integer;
begin
...
function TDOMAttrDef.HasEnumToken(const aValue: AnsiString): Boolean;
var
  I: Integer;
begin
  Result := True;
