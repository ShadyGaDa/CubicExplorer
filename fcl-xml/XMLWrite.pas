...
type
  TSpecialCharCallback = procedure(c: AnsiChar) of object;
  ...
  private
    ...
    procedure wrtChars(Src: PAnsiChar; Length: Integer);
    procedure IncIndent;
    procedure DecIndent; {$IFDEF HAS_INLINE} inline; {$ENDIF}
    procedure wrtStr(const ws: AnsiString); {$IFDEF HAS_INLINE} inline; {$ENDIF}
    procedure wrtChr(c: AnsiChar); {$IFDEF HAS_INLINE} inline; {$ENDIF}
    procedure wrtLineEnd; {$IFDEF HAS_INLINE} inline; {$ENDIF}
    procedure wrtIndent; {$IFDEF HAS_INLINE} inline; {$ENDIF}
    procedure wrtQuotedLiteral(const ws: AnsiString);
    procedure ConvWrite(const s: AnsiString; const SpecialChars: TSetOfChar;
...
procedure TXMLWriter.wrtChars(Src: PAnsiChar; Length: Integer);
var
  pb: PAnsiChar;
  wc: Cardinal;
  SrcEnd: PAnsiChar;
...
begin
  ...
  begin
    ...
    case wc of
      ...
      $80..$7FF: begin
        pb^ := AnsiChar($C0 or (wc shr 6));   Inc(pb);
        pb^ := AnsiChar($80 or (wc and $3F)); Inc(pb);
      end;
      ...
      $D800..$DBFF: begin
        ...
        begin
          ...
          pb^ := AnsiChar($F0 or (wc shr 18));           Inc(pb);
          pb^ := AnsiChar($80 or ((wc shr 12) and $3F)); Inc(pb);
          pb^ := AnsiChar($80 or ((wc shr 6) and $3F));  Inc(pb);
          pb^ := AnsiChar($80 or (wc and $3F));          Inc(pb);
        end
        else
      ...
      begin
        pb^ := AnsiChar($E0 or (wc shr 12));          Inc(pb);
        pb^ := AnsiChar($80 or ((wc shr 6) and $3F)); Inc(pb);
        pb^ := AnsiChar($80 or (wc and $3F));         Inc(pb);
      end;
    end;
...
procedure TXMLWriter.wrtStr(const ws: AnsiString); { inline }
begin
  wrtChars(PAnsiChar(ws), Length(ws));
end;

procedure TXMLWriter.wrtChr(c: AnsiChar); { inline }
begin
  wrtChars(@c,1);
end;
...
begin
  wrtChars(PAnsiChar(FIndent), FIndentCount*2);
end;
...
procedure TXMLWriter.wrtQuotedLiteral(const ws: AnsiString);
var
  Quote: AnsiChar;
begin
...
procedure TXMLWriter.ConvWrite(const s: AnsiString; const SpecialChars: TSetOfChar;
  const SpecialCharCallback: TSpecialCharCallback);
var
...
begin
  ...
  begin
    if (s[EndPos] < #255) and (AnsiChar(s[EndPos]) in SpecialChars) then
    begin
...
procedure TXMLWriter.AttrSpecialCharCallback(c: AnsiChar);
begin
  case c of
    '"': wrtStr(QuotStr);
    '&': wrtStr(AmpStr);
...
procedure TXMLWriter.TextnodeSpecialCharCallback(c: AnsiChar);
begin
  case c of
    '<': wrtStr(ltStr);
