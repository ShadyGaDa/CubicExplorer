...
uses
  ...
  // System Units
  Windows, SysUtils, ActiveX, Registry, Messages, Graphics,
...
procedure TCEBackContextMenu.AddMenuItems(Menu:  hMenu);
  ...
  procedure EnumItem(ToMenu: hMenu; Item: TMenuItem; var ItemID: Integer; Upper: Boolean);
  var
    i,pos: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        // Get Caption
        if chItem is TMenuItem then
        ws:= TMenuItem(chItem).Caption
        else
        ws:= chItem.Caption;
...
//        begin
          ...
          begin
            ...
            info.fType:= MFT_STRING or MFT_RADIOCHECK
            else
            info.fType:= MFT_STRING;
            info.dwTypeData:= PWideChar(ws);
            info.cch:= Length(chItem.Caption) + 1;
            info.wID:= ItemID;
            if not chItem.Enabled then
              info.fState:= MFS_DISABLED	or MFS_GRAYED;
            if chItem.Checked then
              info.fState:= info.fState or MFS_CHECKED;
          end;
          if chItem.Count > 0 then
          begin
            info.fMask:= info.fMask or MIIM_SUBMENU;
...
//        end;
        ...
        // Insert to menu
        InsertMenuItem(ToMenu, pos, true, info);

        // Enum child items
        if chItem.Count > 0 then
        begin

          EnumItem(info.hSubMenu, chItem, ItemID, upper)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            infoA.dwTypeData:= PAnsiChar(String(ws));
...
//    begin
      ...
      begin
        ...
        if GetMenuItemInfoW(AMenu, i, true, info) then
        begin
...
//              begin
                ...
                lpVerbW := MakeIntResource(ResultID)
//              end;
