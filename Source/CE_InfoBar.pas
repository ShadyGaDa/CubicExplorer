...
//  CE_LanguageEngine,
  //CE_Engine,
  // SpTBX
  SpTBXSkins, SpTBXControls,
  // TNT
//  TntGraphics,
  // VSTools
  MPShellUtilities, MPShellTypes, MPCommonObjects,
  // System Units
  ExtCtrls, Controls, Windows, Messages, Contnrs, Graphics, Classes,
...
{$REGION 'TYPES'}
  type
    ...
    protected
      ...
      fThumbWidth: Integer;
      procedure BuildInfoList(AInfo: String = ''); virtual;
    ...
    public
      ...
      function AddInfoItem(AText: String; APrefix: String): Integer; virtual;
      procedure Clear;
      procedure LoadFromPIDL(APIDL: PItemIDList; ASelectionCount: Integer = 1); virtual;
      procedure Paint; override;
      procedure RefreshThumbnail;
      procedure Reload;
      procedure Resize; override;
      property AutoRefreshThumbnail: Boolean read fAutoRefreshThumbnail write fAutoRefreshThumbnail;
      property CalculateHiddenItems: Boolean read fCalculateHiddenItems write fCalculateHiddenItems;
      property Font;
      property LatestNS: TNamespace read fLatestNS;
      property RowHeight: Integer read fRowHeight write fRowHeight;
      property ShowFolderItemCount: Boolean read fShowFolderItemCount write fShowFolderItemCount;
...
{-------------------------------------------------------------------------------
  Add new Info Item
-------------------------------------------------------------------------------}
function TCEInfoBar.AddInfoItem(AText: String; APrefix: String): Integer;
var
  item: TCEInfoItem;
begin
...
{-------------------------------------------------------------------------------
  Build InfoList
-------------------------------------------------------------------------------}
procedure TCEInfoBar.BuildInfoList(AInfo: String = '');
var
  list: TStrings;
  i, c: Integer;
begin
  ...
  try
    ...
    begin
      ...
      begin
        AddInfoItem(list.ValueFromIndex[list.Count - 1] + ' ' + 'Item(s)', '');
        c := list.Count - 2;
      end
      else
        c := list.Count - 1;
...
begin
  ...
  begin
    ...
    try
      ...
      case fIconSize of
        itSmallIcon:
          imgList := SmallSysImages;
        itLargeIcon:
          imgList := LargeSysImages;
        itExtraLargeIcon:
          imgList := ExtraLargeSysImages;
      else
      ...
      BitBlt(ACanvas.Handle, l, t, w, h, bit2.Canvas.Handle, 0, 0, SRCCOPY);  //must b 24 / 32 bbp
    finally
      bit.Free;
      bit2.Free;
    end;
...
begin
  ...
  begin
    StretchBlt(ACanvas.Handle, l, t, w, h, fThumbnailBuffer.Canvas.Handle, 0, 0, fThumbnailBuffer.Width, fThumbnailBuffer.Height, SRCCOPY);
  end;
end;
...
//      size := WideCanvasTextExtent(ACanvas, item.Text);
      size := ACanvas.TextExtent(item.Text);
      ...
      begin
        ...
        size := ACanvas.TextExtent(item.Prefix);
...
//          size := WideCanvasTextExtent(ACanvas, item.Text);
          ...
          begin
            ...
            prefix_size := ACanvas.TextExtent(item.Prefix + ':');
          ...
          // go to row
          if (((r.Right - r.Left) < (size.cx + prefix_size.cx)) and (row < row_num) and (col_num > 1)) or ((col > col_num) and (row <= row_num)) then
          begin
...
{-------------------------------------------------------------------------------
  Load From PIDL
-------------------------------------------------------------------------------}
procedure TCEInfoBar.LoadFromPIDL(APIDL: PItemIDList; ASelectionCount: Integer = 1);
begin
  if csDestroying in Self.ComponentState then
    exit;
...
begin
  ...
  begin
    ...
    fLatestInfoThread.ShowInfoTip := (not (fLatestNS.Folder and fLatestNS.FileSystem)) or fLatestNS.IsParentByNamespace(DesktopFolder, true) or fLatestNS.IsParentByNamespace(DrivesFolder, true) or
...
begin
  ...
  try
    ...
    begin
      ...
      if Info <> '' then
        Info := Info + #13#10 + 'Item(s)' + ':' + IntToStr(i)
      else
        Info := 'Item(s)' + ':' + IntToStr(i);
    end;
