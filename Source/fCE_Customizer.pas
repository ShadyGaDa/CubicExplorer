...
uses
//  TntClasses,
  // CE Units CE_VistaFuncs, CE_Engine,
  dCE_Actions, dCE_Images,
  CE_Utils, CE_Toolbar, CE_TBActions,
  // VirtualTree
  VirtualTrees, VirtualExplorerTree,
  // Toolbar2000
  TB2Dock, //TB2Toolbar, TB2Item, TB2ExtItems,
  // TNT Controls
  ActnList, Forms, CheckLst, StdCtrls,
  // Png Controls
  PngImageList,
  // SpTBXLib
...
//  JvSimpleXml,
  ...
  Dialogs, ImgList, ActiveX, Math, Menus, TB2Item;
...
begin
  ...
  begin
    act:= TAction(CEActions.ActionList.Actions[i]);
  ...
  data.Name:= UTF8Encode('Separators');
  data.IconIndex:= -1;
  data.IsCategory:= true;
  data.IsSeparator:= false;
...
//  label_themeAuthor.Caption:= 'Author' + ': ' + SkinManager.CurrentSkin.SkinAuthor
//  else
//  label_themeAuthor.Caption:= '';
  ...
  group_displayMode.Items.Add('Default');
  group_displayMode.Items.Add('Icon only');
  group_displayMode.Items.Add('Icon above text');
  group_displayMode.Items.Add('Text only');
...
begin
  ...
  data:= ActionTree.GetNodeData(Node);
  if data.IsCategory then
  CellText:= UTF8ToString(data.Name)
  else
  CellText:= data.Name;
end;
...
begin
  ...
  // Populate Theme list
//  SkinManager.SkinsList.GetSkinNames(ThemeList.Items.AnsiStrings);
//  SkinManager.SkinsList.GetSkinNames(ThemeList.Items.Strings);
//  ThemeList.Sorted:= true;
//  i:= ThemeList.Items.IndexOf('Default');
//  if i > -1 then ThemeList.Items.Move(i, 0);
//  ThemeList.ItemIndex:= ThemeList.Items.IndexOf(SkinManager.CurrentSkinName);
...
begin
  ...
  label_help.Caption:= 'Drag buttons to/from toolbars'
  else
  label_help.Caption:= '';
end;
...
{-------------------------------------------------------------------------------
  On Form Key Press
-------------------------------------------------------------------------------}
procedure TCEToolbarCustomizer.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #27) then
  but_close.Click;
end;
