...
uses
  ...
  // Toolbar200
  TB2Toolbar, TB2Dock, TB2Item,
  // SpTBX
  SpTBXItem, SpTBXTabs,
  // TNT
  SysUtils, Classes, ActnList,
  // JVCL
  JvDockVSNetStyle, JvDockInfo, JvDockControlForm,
  JvAppStorage, JvAppXMLStorage, JvSimpleXML,
  // mbTBX
  CEJvDockVSNetStyleTBX,
  // System Units
  Contnrs, Windows, Forms, Messages, Controls, Types;
...
{$REGION 'TYPES'}
  type
    ...
    public
      ...
      procedure EndUpdate;
      function FindDockNamed(AName: AnsiString{!}): TTBDock; overload;
      function FindDockNamed(AName: AnsiString{!}; out IsInner: Boolean): TTBDock; overload;
      function FindInnerDockNamed(AName: AnsiString{!}): TTBDock;
      function FindOuterDockNamed(AName: AnsiString{!}): TTBDock;
    ...
    private
      fAutoSave: Boolean;
      fCurrentLayout: AnsiString{!};
      fFilePath: AnsiString{!};
      procedure InitSelf;
    protected
      CurrentFormLayout: AnsiString{!};
      CurrentInnerToolbarLayout: AnsiString{!};
      CurrentOuterToolbarLayout: AnsiString{!};
    public
      Layouts: TStrings;
      ...
      procedure LoadFromFile(AFilePath: AnsiString{!});
      procedure LoadLayout(LayoutName: AnsiString{!}; LoadInnerToolbarLayout, LoadOuterToolbarLayout, LoadFormLayout: Boolean; ForceReload: Boolean = false);
      procedure LoadSettingsForToolbars;
      procedure SaveCurrentLayout;
      procedure SaveLayout(LayoutName: AnsiString{!}; SaveInnerToolbarLayout, SaveOuterToolbarLayout, SaveFormsLayout: Boolean);
      procedure SaveToFile(AFilePath: AnsiString{!} = '');
      procedure SaveSettingsForToolbars;
      property AutoSave: Boolean read fAutoSave write fAutoSave;
      property CurrentLayout: AnsiString{!} read fCurrentLayout write fCurrentLayout;
...
procedure LoadDockedForms(AppStorage: TJvCustomAppStorage; AppStoragePath: AnsiString{!} = ''; HandleMainForm: Boolean = false);
procedure LoadPositionsForToolbars(AppStorage: TJvCustomAppStorage; AppStoragePath: AnsiString{!} = ''; DockType: TCEToolbarDockType = tdtBoth);
procedure SaveDockedForms(AppStorage: TJvCustomAppStorage; AppStoragePath: AnsiString{!} = '');
procedure SavePositionsForToolbars(AppStorage: TJvCustomAppStorage; AppStoragePath: AnsiString{!} = ''; DockType: TCEToolbarDockType = tdtBoth);

//TOOLBAR
procedure BeginToolbarCustomize;
procedure EndToolbarCustomize;
procedure LoadSettingsForToolbars(AppStorage: TJvAppXMLFileStorage; AppStoragePath: AnsiString{!} = '');
procedure LoadToolbarItems(Toolbar: TSpTBXToolbar; ToolbarNode: TJvSimpleXMLElem);
procedure LoadToolbarProperties(Toolbar: TTBCustomDockableWindow; ToolbarNode: TJvSimpleXMLElem);
procedure SaveSettingsForToolbars(AppStorage: TJvAppXMLFileStorage; AppStoragePath: AnsiString{!} = '');
procedure SaveToolbarItems(Toolbar: TSpTBXToolbar; ToolbarNode: TJvSimpleXMLElem);
procedure SaveToolbarProperties(Toolbar: TTBCustomDockableWindow; ToolbarNode: TJvSimpleXMLElem);

var
  DefaultLayoutName: AnsiString{!} = 'Default';
...
uses
  Main,
  fCE_DockHostForm, fCE_DockableForm,
  CE_SpTabBar, CE_StatusBar, CE_TBActions, CE_ToolbarEditorItems,
  dCE_Actions, dCE_Images,
  SpTBXEditors;
{*------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------}
procedure TCELayoutController.LoadFromFile(AFilePath: AnsiString{!});
var
  s: TStream;
begin
  fFilePath := AFilePath;
  ...
  try
    ...
    begin
      s := TFileStream.Create(fFilePath, fmOpenRead or fmShareDenyNone);
      try
        s.Position := 0;
        try
...
-------------------------------------------------------------------------------}
procedure TCELayoutController.SaveToFile(AFilePath: AnsiString{!} = '');
var
  s: TStream;
begin
  try
...
{*------------------------------------------------------------------------------
  Load layout
-------------------------------------------------------------------------------}
procedure TCELayoutController.LoadLayout(LayoutName: AnsiString{!}; LoadInnerToolbarLayout, LoadOuterToolbarLayout, LoadFormLayout: Boolean; ForceReload: Boolean = false);
var
  s: AnsiString{!};
begin
...
{*------------------------------------------------------------------------------
  Save Layout
-------------------------------------------------------------------------------}
procedure TCELayoutController.SaveLayout(LayoutName: AnsiString{!}; SaveInnerToolbarLayout, SaveOuterToolbarLayout, SaveFormsLayout: Boolean);
var
  s: AnsiString{!};
begin
...
{-------------------------------------------------------------------------------
  Add
-------------------------------------------------------------------------------}
function TCEToolbarDocks.Add(ADock: TTBDock; InnerDock: Boolean = false): Integer;
begin
  if InnerDock then
...
-------------------------------------------------------------------------------}
function TCEToolbarDocks.FindDockNamed(AName: AnsiString{!}): TTBDock;
var
  i: Integer;
begin
  Result := nil;
...
{*------------------------------------------------------------------------------
  Find Dock by it's name
-------------------------------------------------------------------------------}
function TCEToolbarDocks.FindDockNamed(AName: AnsiString{!}; out IsInner: Boolean): TTBDock;
var
  i: Integer;
begin
  Result := nil;
...
-------------------------------------------------------------------------------}
function TCEToolbarDocks.FindInnerDockNamed(AName: AnsiString{!}): TTBDock;
var
  i: Integer;
begin
  Result := nil;
...
-------------------------------------------------------------------------------}
function TCEToolbarDocks.FindOuterDockNamed(AName: AnsiString{!}): TTBDock;
var
  i: Integer;
begin
  Result := nil;
      ...
      begin
        ...
        try
          ...
          begin
            ...
            dockablewindow.FloatingPosition := Point(AppStorage.ReadInteger('DockPos', dockablewindow.FloatingPosition.X), AppStorage.ReadInteger('DockRow', dockablewindow.FloatingPosition.Y));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        else if (DockType = tdtBoth) or ((DockType = tdtOuter) and CEToolbarDocks.IsOuterDock(dockablewindow.CurrentDock)) or ((DockType = tdtInner) and CEToolbarDocks.IsInnerDock(dockablewindow.CurrentDock)) then
        begin
...
begin
  ...
  try
    for I := 0 to Screen.CustomFormCount - 1 do
      if not (Screen.CustomForms[I] is TJvDockableForm) and (Assigned(FinddockClient(Screen.CustomForms[I])) or Assigned(FinddockServer(Screen.CustomForms[I]))) then
